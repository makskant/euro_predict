...
matches = {'finished': {'A':{('FRA', 'ROU') : (2,1), ('ALB', 'SWI') : (0,1), ('ROU', 'SWI') : (1,1), ('FRA', 'ALB') : (2,0)},
                        'B':{('WAL','SVK'):(2,1), ('RUS','SVK'):(1,2),('ENG','RUS'):(1,1), ('ENG','WAL'):(2,1)},
                        'C':{('POL','NIR'):(1,0), ('GER','UKR'):(2,0), ('UKR','NIR'):(0,2), ('POL','GER'):(0,0)},
...
for group in euro['group']:
    for match in euro['group'][group]:
        points[group][match[0]],  points[group][match[1]] = cal_points(match, euro['group'][group][match], points[group])
